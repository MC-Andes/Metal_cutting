"""Coulomb boundary impulses in the MPM background velocity space.

Adapted from direct_q4_candidate/contact.py; the material trace geometry and
Coulomb law are unchanged. GridMechanics supplies the actual grid mobility.
Constraint reactions require a separate physical-support decomposition.
...
from verification.cutting2d.coulomb_scaling_candidate.solver import resolve_coulomb, CoulombAmbiguity
...
def solve_contact(model, q, points, velocity, tool_velocity, dt, fixed, *,
    ...
    while active:
        ...
        for iteration in range(max_iterations):
            ...
            if iteration==19:
                try:
                    impulse,resolution=resolve_coulomb(W,ba,mu,impulse,velocity_tolerance,check_multiple=False)
                    break
                except ContactFailure as exc:
                    semismooth_failures.append(str(exc))
                    # The verified old PGS state and its slack are still intact.
        else:
            try:
                impulse,resolution=resolve_coulomb(W,ba,mu,impulse,velocity_tolerance)
            except CoulombAmbiguity:
                raise
            except ContactFailure as exc:
                semismooth_failures.append(str(exc))
                impulse,resolution=resolve_active_branches(W,ba,mu,impulse,velocity_tolerance)
        if semismooth_failures:resolution['semismooth_failed_attempts']=semismooth_failures
    ...
    reaction_error=model.virtual_work_residual(reaction,model.reaction_envelope(v,old,nodal))
    if reaction_error>1e-11:
        raise ContactFailure('Contact constraint reaction has nonzero admissible grid virtual work')
    ...
    return v,dict(tool_impulse=impulse,tool_angular_impulse=angular(nodal),
        ...
        nodal_contact_impulse=nodal,constraint_reaction=reaction,tool_work=work,
        ...
        free_virtual_work_relative=reaction_error,iterations=iterations,active_set_passes=passes,
